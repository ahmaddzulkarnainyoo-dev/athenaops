...
def fetch_intelligence_data(query):
  ...
  queries = [query, f"{query} twitter", f"{query} tiktok viral"]
  combined_text = ""
  for q in queries:
...
def analyze_ai(system_prompt, user_msg):
  # Membersihkan karakter ghaib \u200b dan memilih model teks generasi yang aktif di Groq
  selected_model = "qwen/qwen3.8-27b".replace("\u200b", "").strip()
...
with st.sidebar:
  ...
  mode = st.radio(
      "Operasi:", ["Private War Room", "Public Intelligence Radar"]
  )
  st.markdown("---")
...
if mode == "Private War Room":
  ...
  with col1:
    ...
    archetype = st.selectbox(
        "Archetype:",
        ["Narcissist", "Tyrant", "Seducer", "Shadow Player", "Weak Leader"],
    )
    insecurity = st.text_input("The Thumbscrew (Kelemahan):")
    dynamic = st.select_slider(
        "Power Dynamic:", options=["Dia Dominan", "Seimbang", "User Dominan"]
    )
...
"""
            user_msg = (
                f"Target: {target}, Archetype: {archetype}, Kelemahan:"
                f" {insecurity}, Goal: {goal}, Evidence: {evidence}"
            )
            result = analyze_ai(sys_msg, user_msg)
            st.markdown(result)
            st.markdown(
                get_binary_file_downloader_html(result, f"Analisis_{target}"),
                unsafe_allow_html=True,
            )
...
# MODE 2
  ...
  tab1, tab2 = st.tabs(
      [" Social Narrative Scanner", "🔍 Framing Forensic"]
  )

  with tab1:
    st.subheader("Automated Cross-Platform Scan")
    st.caption(
        "*Data bersumber dari Google News index. Direct scraping Twitter/TikTok"
        " memerlukan API terpisah."
    )
    query = st.text_input(
        "Masukkan Nama Tokoh / Isu Viral:", placeholder="Misal: @PolitisiX atau #KasusY"
    )
    if st.button("Deep Scan Medsos & Berita"):
      if query:
        with st.spinner(
            f"Menjelajah X, TikTok, dan Portal Berita untuk '{query}'..."
        ):
          try:
...
"""
              ...
              st.markdown(
                  get_binary_file_downloader_html(analysis, f"Radar_{query}"),
                  unsafe_allow_html=True,
              )
            else:
    ...
    if st.button("Detect Manipulation"):
      if not news_text:
        ...
        with st.spinner("Membedah framing..."):
          try:
            analysis = analyze_ai(
                "Bedah framing, bias, dan agenda tersembunyi dari teks ini"
                " secara detail.",
                news_text,
            )
            st.markdown(analysis)
            st.markdown(
                get_binary_file_downloader_html(analysis, "Framing_Report"),
                unsafe_allow_html=True,
            )
          except Exception as e:
            st.error(f"Analisis gagal: {str(e)}")
